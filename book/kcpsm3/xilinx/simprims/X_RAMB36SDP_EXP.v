// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/xec_libs/data/simprims/X_RAMB36SDP_EXP.v,v 1.4 2008/10/02 19:01:56 vandanad Exp $
///////////////////////////////////////////////////////////////////////////////
...
module X_RAMB36SDP_EXP (DBITERR, DO, DOP, ECCPARITY, SBITERR, 
    ...
    X_ARAMB36_INTERNAL INT_RAMB (.DIA(64'b0), .ENA(RDENL), .WEA(8'b0), .SSRA(SSRL), .ADDRA(RDADDRL), .CLKA(RDCLKL), .DOA(DO), .DIB(DI), .ENB(WRENL), .WEB(WEL), .SSRB(1'b0), .ADDRB(WRADDRL), .CLKB(WRCLKL), .DOB(dangle_out32), .DOPA(DOP), .DOPB(dangle_out4), .DIPA(4'b0), .DIPB(DIP), .CASCADEOUTLATA(dangle_out), .CASCADEOUTLATB(dangle_out), .CASCADEOUTREGA(dangle_out), .CASCADEOUTREGB(dangle_out), .CASCADEINLATA(1'b0), .CASCADEINLATB(1'b0), .CASCADEINREGA(1'b0), .CASCADEINREGB(1'b0), .REGCEA(REGCEL), .REGCEB(1'b0), .REGCLKA(RDRCLKL), .REGCLKB(1'b0), .DBITERR(DBITERR), .ECCPARITY(ECCPARITY), .SBITERR(SBITERR));
